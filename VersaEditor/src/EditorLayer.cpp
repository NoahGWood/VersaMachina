...
namespace VersaMachina
{
    ...
    void EditorLayer::GetHoveredEntity()
    {
...
#pragma GCC diagnostic pop
            ...
            if (ImGui::BeginDragDropTarget())
            {
                if (const ImGuiPayload *payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_ITEM"))
                {
                    ...
                    {
                        // Grab the hovered entity
                        // Get pixel
                        if(*m_SelectedEntity && m_SelectedEntity->HasComponent<ECS::SpriteRendererComponent>())
                        {
                            ECS::SpriteRendererComponent sprite = m_SelectedEntity->GetComponent<ECS::SpriteRendererComponent>();
                            VM_CORE_INFO("CREATING TEXTURE: {0}", path.c_str());
                            sprite.Texture = Render::Texture2D::Create(path.c_str());                            
                        }
                        // Select transform
    ...
    {
        auto mouse = e.GetMouseButton();
        if (mouse == Mouse::ButtonLeft && m_ViewportHovered && !ImGuizmo::IsUsingAny() && !ImGuizmo::IsOver())
        {

            *m_SelectedEntity = m_HoveredEntity;
        } else if(m_ContentBrowserPanel.m_Hovered || m_ContentBrowserPanel.m_Focused)
        {
            switch (mouse)
            {
                case Mouse::Button3 : m_ContentBrowserPanel.BackButton(); break;
                case Mouse::Button4 : m_ContentBrowserPanel.NextButton(); break;
            }
        }
